...
describe('Clarification Recovery E2E', () => {
  ...
  it('should recover state and set pendingComplaintType to AMBIGUOUS_LOST_ITEM when session is corrupted', async () => {
    const sess = `sess-clarify-recovery-${Date.now()}`;

    // Create session in DB or cache with step COMPLAINT_LOST_ITEM_CLARIFICATION but no pendingComplaintType
    const state = await chatService.getOrCreateSession(sess);
    state.workflow = 'complaint';
    state.step = 'COMPLAINT_LOST_ITEM_CLARIFICATION';
    state.pendingComplaintType = undefined;
