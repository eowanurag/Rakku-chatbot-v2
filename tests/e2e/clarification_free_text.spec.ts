...
describe('Clarification Free Text E2E', () => {
  ...
  it('should parse free text inside clarification state: "My Aadhaar card" -> Lost Document', async () => {
    const sess = `sess-clarify-freetext-${Date.now()}`;
    ...
    // Free text input inside clarification state
    const res = await chatService.sendMessage('My Aadhaar card', sess);
