...
describe('Lost Item Clarification E2E', () => {
  ...
  it('should trigger clarification on ambiguous container and map selection', async () => {
    const sess = `sess-lost-item-clarify-${Date.now()}`;
    ...
    // Initial ambiguous container phrase
    const res = await chatService.sendMessage('I lost my purse', sess);
    expect(res.response).toContain('To help you file the correct complaint, please tell me what was inside it');
    expect(res.response).toContain('Important Documents');
    expect(res.response).toContain('Mobile Phone');

    // Selection 2: Mobile Phone
    const nextRes = await chatService.sendMessage('2', sess);
