...
describe('Cross District Matrix E2E Spec', () => {
  ...
  const testCases = [
    { citizen: 'Meerut', incident: 'Lucknow' },
    { citizen: 'Lucknow', incident: 'Noida' },
    { citizen: 'Noida', incident: 'Ghaziabad' },
    { citizen: 'Ayodhya', incident: 'Varanasi' }
  ];

  for (const tc of testCases) {
    it(`should allow citizen from ${tc.citizen} to submit incident in ${tc.incident}`, async () => {
      const sess = `sess-matrix-${tc.citizen}-${tc.incident}-${Date.now()}`;
      ...
      await chatService.sendMessage(tc.citizen, sess);
      await chatService.sendMessage('Confirm', sess);
      await chatService.sendMessage(`Area in ${tc.citizen}`, sess);
      ...
      await chatService.sendMessage(`Near station, ${tc.incident}`, sess); // Incident Location
      await chatService.sendMessage('01/01/2026', sess); // Incident Date
      
      const reviewRes = await chatService.sendMessage(`lost phone in ${tc.incident}`, sess);
