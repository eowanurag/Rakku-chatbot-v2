...
describe('Mixed Item Classification E2E', () => {
  ...
  it('should route wallet with phone and cards to Lost Mobile / Theft with bank recommendations', async () => {
    const sess = `sess-mixed-direct-${Date.now()}`;
    ...
    // Wallet with phone and cards
    const res = await chatService.sendMessage('I lost my wallet containing phone and cards', sess);
    ...
    expect(state.data.entities.mixedContents).toBe(true);
    expect(state.data.entities.containsCards).toBe(true);
    expect(state.data.entities.containsPhone).toBe(true);
