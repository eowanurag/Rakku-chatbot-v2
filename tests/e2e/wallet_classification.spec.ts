...
describe('Wallet Classification E2E', () => {
  ...
  it('should route "I lost my wallet containing Aadhaar" directly to Lost Document', async () => {
    const sess = `sess-wallet-direct-${Date.now()}`;
    ...
    // Initial phrase contains wallet + aadhaar
    const res = await chatService.sendMessage('I lost my wallet containing Aadhaar', sess);
    expect(res.response).toContain('I understand that you would like to file a Lost Document complaint')
      .or.toContain('I believe this may be a Lost Document complaint');
    ...
    expect(state.data.lostItemContents).toBe('Documents');
