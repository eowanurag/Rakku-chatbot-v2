...
describe('Purse with Cash E2E', () => {
  ...
  it('should skip clarification and route to Lost Document with propertyOnly and GD recommendations when purse contains cash: "I lost my purse containing ₹5000."', async () => {
    const sess = `sess-purse-cash-${Date.now()}`;
    ...
    const res = await chatService.sendMessage('I lost my purse containing ₹5000.', sess);
    ...
    expect(state.data.lostItemContents).toBe('Cash Only');
    expect(state.intelligence.secondaryRecommendations).toContain('File General Diary at Police Station');
  });
});
