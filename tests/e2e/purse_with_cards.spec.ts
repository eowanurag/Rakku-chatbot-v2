...
describe('Purse with Cards E2E', () => {
  ...
  it('should skip clarification when user specifies cards: "I lost my purse. My purse had ATM cards."', async () => {
    const sess = `sess-purse-cards-${Date.now()}`;
    ...
    const res = await chatService.sendMessage('I lost my purse. My purse had ATM cards.', sess);
    expect(res.response).toContain('Cyber Fraud / Financial Loss');
    ...
    expect(nextRes.response).toContain('where the incident occurred');

    const state = await chatService.getOrCreateSession(sess);
    expect(state.data.type).toBe('Cyber Fraud / Financial Loss');
    expect(state.data.lostItemContents).toBe('ATM / Debit / Credit Cards');
    expect(state.data.entities.lostContainerType).toBe('PURSE');
    expect(state.intelligence.secondaryRecommendations).toContain('Block ATM cards');
  });
});
