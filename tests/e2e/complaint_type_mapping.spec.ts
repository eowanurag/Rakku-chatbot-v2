...
describe('Complaint Type Mapping E2E Spec', () => {
  ...
  it('should store complaint type in session.data.type and keep session.data.location clear during type selection', async () => {
    const sess = `sess-type-mapping-${Date.now()}`;
    ...
    expect(state.data.location).toBeUndefined();
  }, 30000);
});
