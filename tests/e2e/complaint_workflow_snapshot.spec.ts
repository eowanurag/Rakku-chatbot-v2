...
describe('Complaint Workflow Snapshot E2E Spec', () => {
  ...
  it('should verify complete happy path: Mobile -> Name -> Location -> Address -> Profile Confirmation -> Complaint Type -> Incident Date -> Description -> Review -> Ready for Submission', async () => {
    const sess = `sess-snapshot-${Date.now()}`;
    ...
    await chatService.sendMessage('Gomti Nagar Crossing', sess); // Incident Location
    ...
    // Retrieve state to assert conditions
    const state = await chatService.getOrCreateSession(sess);
    expect(state.data.type).toBe('Lost Mobile / Theft');
    expect(state.data.location).toBe('Gomti Nagar Crossing');
    expect(state.data.type).not.toBe(state.data.location);
  }, 30000);
});
