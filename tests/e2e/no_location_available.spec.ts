...
describe('No Location Available E2E Spec', () => {
  ...
  it('should trigger manual entry when location input has invalid characters only', async () => {
    const sess = `sess-no-loc-${Date.now()}`;
    ...
    // Send short invalid location
    const res = await chatService.sendMessage('xy', sess);

    // Should NOT confirm Lucknow or Hazratganj
    expect(res.response).not.toContain('Lucknow');
    expect(res.response).not.toContain('Hazratganj');

    // Should prompt for manual entry in a different way
